...
<body class="sidebar-noneoverflow">
    ...
    <div class="header-container fixed-top">
        <header class="header navbar navbar-expand-sm">
            <ul class="navbar-nav theme-brand flex-row  text-center">
                ...
                <li class="nav-item theme-text" style="width: 120px">
                    <a href="" class="nav-link"> SMPN 270 </a>
                </li>
                            ...
                            <div class="notification-scroll">
                                @if ($notif_tugas->count() > 0 || $notif_materi->count() > 0 || $notif_ujian->count() > 0)
                                    ...
                                    @foreach ($notif_ujian as $nu)
                                        @if ($nu->ujian->jenis == 0)
                                            <a href="{{ url('siswa/ujian/' . $nu->kode) }}" class="klik-notif-ujian" data-sudah="@if($nu->waktu_berakhir == null) true @endif" data-waktu_ujian="{{ $nu->ujian->waktu_ujian }}">
                                                <div class="dropdown-item">
                                        ...
                                        @else
                                            <a href="{{ url('siswa/ujian_essay/' . $nu->kode) }}" class="klik-notif-ujian" data-sudah="@if($nu->waktu_berakhir == null) true @endif" data-waktu_ujian="{{ $nu->ujian->waktu_ujian }}">
                                                <div class="dropdown-item">
                                                    <div class="media file-upload">
                                                        <span data-feather="cast"></span>
                                                        <div class="media-body">
    ...
    <script>
        ...
        $(".klik-notif-ujian").click(function(e){
            e.preventDefault();

            var waktu_ujian = $(this).data('waktu_ujian');
            var sudah = $(this).data('sudah');
            var batas_waktu = new Date(waktu_ujian).getTime();
            ...
            if (waktu_sekarang > batas_waktu) {
                ...
                swal({
                    title: 'Warning!',
                    text: 'Belum saatnya ujian!, Ujian bisa di akses pada ' + waktu_ujian,
                    type: 'warning',
                    padding: '2em'
                });
            }
